...
const trackAccentColor = (name: string): string => {
  ...
  const palette = ['#c8222a','#1565c0','#2e7d32','#e65100','#6a1b9a','#00838f','#c62828','#4527a0'];
...
const isAudio = (name?: string) =>
  !!name && ['.mp3','.wav','.flac','.ogg','.aac','.m4a'].some(ext => name.toLowerCase().endsWith(ext));

const stripExt = (name: string) => name.replace(/\.(mp3|wav|flac|ogg|aac|m4a)$/i, '');
      ...
      if (meta.common.picture?.length) {
        ...
        try {
          ...
          if (id === loadIdRef.current) {
            ...
            const dominant = (facColor && !facColor.error) ? facColor.hex : (palette[1] || '#c8222a');
            setThemeColor(dominant);
            setThemeText(isHexDark(dominant) ? '#ffffff' : '#111111');
          }
        } catch (_) {}
      } else {
        if (id === loadIdRef.current) {
  ...
  const renderSmartPills = () => (
    <div className="player-smart-section">
      ...
      <div className="player-taste-pills">
        {TASTES.map(t => (
          <button key={t.id} className={`taste-pill ${smartTaste === t.id ? 'active' : ''}`}
    ...
    <div className="studio-dashboard fade-in">
      ...
      <div className="dsp-grid">
        ...
        <div className="dsp-card">
          <div className="dsp-label-row"><label>Harmonic Exciter</label><span className="val-green">{upscaleDrive.toFixed(1)}×</span></div>
        ...
        <div className="dsp-card">
          <div className="dsp-label-row"><label>Reverb</label><span className="val-orange">{Math.round(reverbWet*100)}%</span></div>
  ...
  const renderExpandedControls = () => {
    ...
    return (
      <div className="ep-controls-section">
        ...
        {/* RESTORED: All action buttons perfectly flush with progress container */}
        <div className="ep-actions">
          <button className="ep-icon-btn" onClick={toggleFavorite}
            style={{ color: isCurrentFavorite ? 'var(--theme-color)' : undefined }}>
            {isCurrentFavorite ? '♥' : '♡'}
          </button>
          <button className={`ep-icon-btn ${showLyrics ? 'active-glow' : ''}`}
            onClick={e => { 
              e.stopPropagation(); 
              setShowLyrics(!showLyrics); 
              setShowStudio(false); 
            }}
            style={{ color: showLyrics ? 'var(--theme-color)' : undefined }} title="Lyrics">
          ...
          <button className={`ep-icon-btn ${showStudio ? 'active-glow' : ''}`}
            ...
            style={{ color: showStudio ? '#00e676' : undefined }} title="Fine Tune DSP">
  ...
  const renderMobileHeader = () => (
    <div className="mobile-header">
      ...
      <div className="mobile-header-actions">
        <button className="mobile-icon-btn" onClick={() => setMobileSearchOpen(s => !s)} title="Search">
        ...
        <button className="mobile-icon-btn theme-toggle-btn" onClick={toggleTheme} title="Toggle theme">
          {isDarkMode
        ...
        <button className="mobile-icon-btn add-folder-mobile" onClick={handleBrowseFolder} disabled={isLoading} title="Add Folder">
          {isLoading ? '⏳' : '+'}
        </button>
      </div>
    </div>
  ...
  const renderMobileTabs = () => (
    <div className="mobile-tabs">
      <button className={currentView==='ALL'?'active':''} onClick={() => setCurrentView('ALL')}>
        🎵 Tracks <span className="tab-count">{playlist.length}</span>
      </button>
      ...
      <button className={currentView==='BOLLYWOOD'?'active':''} onClick={() => setCurrentView('BOLLYWOOD')}>
        🎙️ Bollywood <span className="tab-count">{bollywoodCount}</span>
      </button>
    </div>
  );
    ...
    >
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          <button className="add-folder-btn" onClick={handleBrowseFolder} disabled={isLoading}>{isLoading ? '⏳ Scanning…' : '+ Add Folder'}</button>
            ...
            <>
              ...
              <ul className="track-list">
                {displayedTracks.map((track, index) => {
                  ...
                  return (
                    <li key={index} className={`track-item ${isActive ? 'active' : ''}`} style={{ '--track-color': trackAccentColor(track.name) } as React.CSSProperties} onClick={() => playTrack(track)}>
        ...
        <footer className={`bottom-player ${isExpanded ? 'expanded' : ''}`}>
              ...
              <div className="player-interface">
                <div className="track-info" onClick={() => setIsExpanded(true)}>
                  ...
                  <div className="mini-text-block">
                    {(() => {
                      const titleLong = trackTitle.length > 22;
                      return (
                        <div className={`mini-marquee-clip ${titleLong ? 'scrolling' : ''}`}>
                          <div className={`mini-marquee-inner ${titleLong ? 'running' : ''}`}>
                            <span className="track-title">{trackTitle}</span>
                            {titleLong && <span className="track-title">{trackTitle}</span>}
                          </div>
                        </div>
                      );
                    })()}
                    {(() => {
                      const artistText = trackArtist + (detectedProfile ? ` ${detectedProfile.icon}` : '');
                      const artistLong = artistText.length > 26;
                      return (
                        <div className={`mini-marquee-clip ${artistLong ? 'scrolling' : ''}`}>
                          <div className={`mini-marquee-inner ${artistLong ? 'running' : ''}`}>
                            <span className="artist-subtitle">{trackArtist}{detectedProfile && <span className="mini-profile"> {detectedProfile.icon}</span>}</span>
                            {artistLong && <span className="artist-subtitle">{trackArtist}{detectedProfile && <span className="mini-profile"> {detectedProfile.icon}</span>}</span>}
                          </div>
                        </div>
                      );
                    })()}
          ...
          {isExpanded && (
            <div className="expanded-player-content fade-in">
              ...
              {/* CORE FIX: Lyrics and Art dynamically swap in the exact same view */}
              <div className={`ep-content ${isRightPaneActive ? 'lyrics-mode' : ''}`} style={{ position:'relative', zIndex:10 }}>
                
                <div className="ep-left">
                  <div className="ep-art"
                    style={{ backgroundImage: albumArt ? `url(${albumArt})` : 'none', backgroundColor: 'rgba(128,128,128,0.08)' }}
                    onClick={() => {
                      if (window.innerWidth <= 768 && lyrics.length > 0) {
                        setShowLyrics(true);
                        setShowStudio(false);
                      }
                    }}
                  >
                    {!albumArt && <span>🎵</span>}
                    {lyrics.length > 0 && (
                      <div className="lyrics-art-badge">
