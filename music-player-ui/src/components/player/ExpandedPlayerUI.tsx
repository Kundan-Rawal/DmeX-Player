...
export const DSPStudio = ({
  ...
  const applyPreset = async (preset:'STUDIO'|'CINEMATIC'|'RELAX') => {
    ...
    else if(preset==='CINEMATIC'){pRem=true;pCmp=true;pDrv=1.2;pWid=1.35;p3D=0.25;pDep=0.4;pRvb=0.16;pBas=0.8;pTrb=0.5;}
    else{p3D=0.40;pDep=0.6;pRvb=0.22;pBas=0.1;pTrb=0.1;}
    setIsRemastered(pRem);setIsCompressed(pCmp);setUpscaleDrive(pDrv);setWidenWidth(pWid);setSpatialExtra(p3D);setDepthAmount(pDep);setReverbWet(pRvb);setBassLevel(pBas);setTrebleLevel(pTrb);
